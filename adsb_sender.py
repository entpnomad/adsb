...
def connect_to_dump1090(host: str, port: int) -> socket.socket:
    try:
        ...
        print(f"[sender] Connected to dump1090 at {host}:{port}")
        return sock
    except (socket.error, OSError) as exc:
        print(f"[sender] Failed to connect to {host}:{port}: {exc}")
        raise
    ...
    while True:
        try:
            ...
            with sock.makefile("r", encoding="utf-8", errors="replace") as f:
                print(f"[sender] Streaming SBS-1 and forwarding to {ingest_url}")
                for line in f:
                    ...
                    if total - last_log >= FLUSH_INTERVAL:
                        print(f"[sender] Forwarded {total} positions...", end="\r")
                        last_log = total
